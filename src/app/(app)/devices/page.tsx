  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">端末登録</h1>
          <p className="text-slate-500 text-sm mt-1">{rows.length}台</p>
        </div>
        <Link
          href="/devices/new"
      ...
      {rows.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-16 text-center">
          ...
          <p className="text-slate-600 font-medium">端末がまだ登録されていません</p>
          <Link
            href="/devices/new"
            ...
            <div key={d.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-1">{d.members?.name ?? '担当者未設定'}</p>
                </div>
                {d.owner_type && (
              ...
              {d.usage_note && <p className="text-slate-600 text-sm mt-2">{d.usage_note}</p>}
            </div>
          ))}
        </div>
      )}
