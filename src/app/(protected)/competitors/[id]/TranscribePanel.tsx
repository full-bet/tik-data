...
export default function TranscribePanel({
  ...
  return (
    <div className="space-y-3">
      {/* ファイルアップロードエリア */}
      <div
        className="border-2 border-dashed border-slate-200 hover:border-indigo-300 rounded-lg p-5 text-center cursor-pointer transition-colors"
      ...
      >
        ...
        {status === 'uploading' || status === 'saving' ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-500">MP4 / 音声ファイルをアップロード</p>
            <p className="text-xs text-slate-400 mt-0.5">クリックまたはドラッグ＆ドロップ</p>
          </>
        )}
      </div>
