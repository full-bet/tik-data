...
export default async function CompetitorDetailPage({
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-6">
        <Link href="/competitors" className="text-slate-400 hover:text-slate-600 text-sm">
          ← 競合分析
        </Link>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-5">
        <div className="flex gap-4">
          {comp.thumbnail_url ? (
            ...
            <div className="w-20 h-20 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 text-3xl">
              🎵
            </div>
          )}
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-500 mt-1">{comp.creator_name ?? '—'}</p>
            <a
              href={comp.url}
              target="_blank"
      ...
      {/* 文字起こし */}
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-5">
        <h2 className="font-semibold text-slate-800 mb-4">文字起こし（Whisper）</h2>
      ...
      {comp.transcript && (
        <div className="bg-white rounded-xl border border-slate-200 p-6 mb-5">
          <h2 className="font-semibold text-slate-800 mb-4">AI 構成分析</h2>
          ...
          {comp.ai_summary && (
            <div className="mt-5 space-y-4">
              ...
              <div className="bg-indigo-50 rounded-lg p-4">
                <p className="text-xs font-semibold text-indigo-500 mb-1">まとめ</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {comp.hook && (
                  <div className="bg-amber-50 rounded-lg p-3">
                    <p className="text-xs font-semibold text-amber-600 mb-1">🪝 冒頭フック</p>
              ...
              {structure.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-slate-500 mb-2">📋 構成</p>
      ...
      {/* 手動メモ */}
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-5">
        <h2 className="font-semibold text-slate-800 mb-3">メモ</h2>
...
function NotesForm({ competitorId, defaultValue }: { competitorId: string; defaultValue: string }) {
  ...
  return (
    <form action={save}>
      ...
      <button
        type="submit"
        className="mt-2 px-4 py-1.5 bg-slate-800 text-white rounded-lg text-xs font-medium hover:bg-slate-900 transition-colors"
      >
        保存
      </button>
    </form>
